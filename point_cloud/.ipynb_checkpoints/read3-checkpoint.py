...
def create_walkability_map(ply_path, grid_size=0.1, height_threshold=11.5):
    ...
    point_count = np.zeros((len(x_bins) - 1, len(z_bins) - 1), dtype=int)  # New array for point
...
def visualize_map(walkable_grid):
    ...
    ax.set_title('Robot Walkability Map', fontsize=16)
...
if __name__ == "__main__":
    ...
    walkable, origin, used_grid_size = create_walkability_map(
        ply_file, grid_size=cell_size, height_threshold=max_height_var
    )
